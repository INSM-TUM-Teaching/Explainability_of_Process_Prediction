...
interface RemainingTimeGlobalResultsProps {
  runId: string;
  datasetId: string;
  summary: any;
...
export default function RemainingTimeGlobalResults({ runId, datasetId, summary, onCaseClick }: RemainingTimeGlobalResultsProps) {
    ...
    <div className="flex flex-col gap-6 w-full">
      ...
      <div className="flex flex-col gap-4 mt-4">
        ...
        <div className="grid grid-cols-1 md:grid-cols-4 gap-4 mb-2">
          ...
          <div className="rounded border bg-white p-4 text-center shadow-sm">
              <div className="text-sm text-slate-500 uppercase tracking-wide">Mean Actual</div>
              <div className="mt-1 text-2xl font-semibold text-slate-700">{(globalStats.mean_actual || 0).toFixed(2)} d</div>
          </div>
          <div className="rounded border bg-white p-4 text-center shadow-sm">
              <div className="text-sm text-slate-500 uppercase tracking-wide">Mean Predicted</div>
              <div className="mt-1 text-2xl font-semibold text-slate-700">{(globalStats.mean_predicted || 0).toFixed(2)} d</div>
          </div>
        </div>
        ...
        <div className="rounded border bg-white p-4 shadow-sm h-[400px]">
          <h3 className="text-md font-semibold mb-4 text-brand-900">Top 10 Variants - Error vs Volume</h3>
        ...
        {/* Top 10 Variants with Highest Error */}
        <div className="rounded border bg-white p-4 shadow-sm h-[400px]">
          <h3 className="text-md font-semibold mb-4 text-brand-900">Top 10 Variants with Highest Error</h3>
          <ResponsiveContainer width="100%" height="100%">
      ...
      {(globalStats.global_explanations?.shap_features?.length > 0 || globalStats.global_explanations?.lime_features?.length > 0 || globalStats.global_explanations?.gnn_features?.length > 0 || (summary?.request?.model_type === "best" && topPatterns.length > 0)) && (
        ...
        {globalStats.global_explanations && globalStats.global_explanations.shap_features && globalStats.global_explanations.shap_features.length > 0 && (
          <DynamicHorizontalBarChart 
            ...
            task={summary?.request?.task || "remaining_time"}
            method="SHAP"
            isGlobal={true}
          />
        )}
            ...
            <table className="w-full text-sm">
              <thead>
                <tr className="border-b bg-slate-50">
                  ...
                  <th className="text-center py-2 px-3 font-semibold text-slate-700">MAE (Days)</th>
                </tr>
              </thead>
              <tbody>
                {filteredPatterns.slice(0, 20).map((pattern: any, idx: number) => {
                  ...
                  const mae = parseFloat(pattern.global_accuracy) ? parseFloat(pattern.global_accuracy).toFixed(2) : "0.00";
                  ...
                  return (
                    ...
                    >
                      ...
                      <td className="py-2 px-3 text-center">
                        <span className="text-red-600 font-semibold">{mae}</span>
                      </td>
                    </tr>
                  );
                })}
