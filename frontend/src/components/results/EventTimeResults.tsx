...
import EventTimeGlobalResults from "./EventTimeGlobalResults";
...
type PredictionRecord = {
  ...
  case_index: number;
  sequence: string;
  actual_event_time_days?: number;
  true_event_time_days?: number;
  predicted_event_time_days: number;
  confidence_percent?: number;
  confidence?: number;
  variant_id?: string | number;
};

export default function EventTimeResults({ runId, summary, uploadedFileName, configMode }: any) {
  ...
  useEffect(() => {
    async function loadData() {
      ...
      } else if (summary.request.model_type === "transformer" && (summary.request.task === "event_time" || summary.request.task === "time" || summary.request.task === "event_time")) {
    ...
    <div className="flex flex-col gap-6 w-full">
      ...
      <div className="flex flex-col gap-4 rounded-2xl border border-brand-100 bg-white p-6 shadow-sm md:flex-row md:items-center md:justify-between">
        <div className="flex-1 min-w-0 pr-4">
          ...
          <div className="mt-3 text-sm text-slate-600 flex flex-wrap items-center gap-x-3 gap-y-1">
            ...
            <span className="text-slate-300">|</span>
            <span>Prediction Task: <strong className="capitalize">{summary.request?.task?.replace('_', ' ') || "Event Time"}</strong></span>
            <span className="text-slate-300">|</span>
      ...
      <Tabs value={activeTab} onValueChange={setActiveTab} className="w-full mt-4">
        ...
        <TabsContent value="global">
          <EventTimeGlobalResults runId={runId} datasetId={summary.dataset?.dataset_id} summary={summary} onCaseClick={handleCaseClick} />
        </TabsContent>
...
function EventTimeCasePredictionBlock({ caseId, records, runId, modelType, explainabilityType, autoExpand = false }: { caseId: string, records: any[], runId: string, modelType: string, explainabilityType?: string, autoExpand?: boolean }) {
  ...
  const trueVal = Number(selectedRecord.actual_event_time_days || selectedRecord.true_event_time_days || 0);
  const predVal = Number(selectedRecord.predicted_event_time_days || 0);
  const elapsedVal = Number((selectedRecord as any).current_elapsed_time_days || 0);
        ...
        <div className="p-4 bg-white border-t text-sm flex flex-col gap-4">
          ...
          <div className="grid grid-cols-4 gap-4">
            ...
            <div className="p-3 border rounded">
              <div className="text-slate-500 mb-1">True Event Time</div>
            ...
            <div className={`p-3 border rounded ${isClose ? 'bg-green-50 border-green-200' : 'bg-red-50 border-red-200'}`}>
              <div className={`mb-1 ${isClose ? 'text-green-700' : 'text-red-700'}`}>Predicted Event Time</div>
                  ...
                  <div>
                    {explainResult.localData ? (
                      <DynamicHorizontalBarChart 
                        ...
                        task={explainResult.localData.task || "event_time"} 
